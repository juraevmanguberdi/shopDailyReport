<template>
  <div>
    <h2 id="page-heading" data-cy="ExpenseHeading">
      <span v-text="$t('shopDailyReportApp.expense.home.title')" id="expense-heading">Expenses</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('shopDailyReportApp.expense.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ExpenseCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('shopDailyReportApp.expense.home.createLabel')"> Create a new Expense </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && expenses && expenses.length === 0">
      <span v-text="$t('shopDailyReportApp.expense.home.notFound')">No expenses found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('expenseDate')">
              <span v-text="$t('shopDailyReportApp.expense.expenseDate')">Expense Date</span>
            ...
            <th scope="row" v-on:click="changeOrder('notes')">
              <span v-text="$t('shopDailyReportApp.expense.notes')">Notes</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="shopDailyReportApp.expense.delete.question" data-cy="expenseDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-expense-heading" v-text="$t('shopDailyReportApp.expense.delete.question', { id: removeId })">
