/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import PaymentMethodDetailComponent from '@/entities/payment-method/payment-method-details.vue';
import PaymentMethodClass from '@/entities/payment-method/payment-method-details.component';
import PaymentMethodService from '@/entities/payment-method/payment-method.service';
import router from '@/router';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('PaymentMethod Management Detail Component', () => {
    let wrapper: Wrapper<PaymentMethodClass>;
    let comp: PaymentMethodClass;
    let paymentMethodServiceStub: SinonStubbedInstance<PaymentMethodService>;

    beforeEach(() => {
      paymentMethodServiceStub = sinon.createStubInstance<PaymentMethodService>(PaymentMethodService);

      wrapper = shallowMount<PaymentMethodClass>(PaymentMethodDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { paymentMethodService: () => paymentMethodServiceStub, alertService: () => new AlertService() },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundPaymentMethod = { id: 123 };
        paymentMethodServiceStub.find.resolves(foundPaymentMethod);

        // WHEN
        comp.retrievePaymentMethod(123);
        await comp.$nextTick();

        // THEN
        expect(comp.paymentMethod).toBe(foundPaymentMethod);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundPaymentMethod = { id: 123 };
        paymentMethodServiceStub.find.resolves(foundPaymentMethod);

        // WHEN
        comp.beforeRouteEnter({ params: { paymentMethodId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.paymentMethod).toBe(foundPaymentMethod);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
